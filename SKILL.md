...
- `TOTAL=<项目总数>`
- 每个项目一组：`PROJECT_<i>_ID` / `PROJECT_<i>_TITLE` / `PROJECT_<i>_DESC`（项目简介） / `PROJECT_<i>_COVER`（封面图 URL） / `PROJECT_<i>_CHARITY` / `PROJECT_<i>_AMOUNT`（已筹金额） / `PROJECT_<i>_COUNT`（已筹人次） / `PROJECT_<i>_REGISTERED_NO`（备案编号） / `PROJECT_<i>_DETAIL`（详情链接，仅供后端生成二维码用，不直接展示） / `PROJECT_<i>_DETAIL_QR_URL`（详情二维码页面 URL，供用户点击跳转） / `PROJECT_<i>_MONTH`（月捐标识 Y/N）
...
- **项目详情二维码**：把 `PROJECT_<i>_DETAIL_QR_URL` 渲染成一个**可点击的链接**（如 `[📱 扫码查看项目详情](<PROJECT_<i>_DETAIL_QR_URL>)`），引导用户**点击链接**打开二维码页面，再用**手机扫码**在手机端查看项目详情。
  - ⚠️ **不要**用 markdown 图片语法 `![...](<PROJECT_<i>_DETAIL_QR_URL>)` 内嵌该 URL：Agent 客户端的 CSP 会拦截外部图片域名，导致二维码「第一次不显示」。
  - 该 URL 指向的后端页面已把二维码以 `data:image/png;base64,...` 内嵌在 HTML 里，由浏览器直接渲染——既绕开 CSP（导航不受 `img-src` 限制），又不让 base64 进入上下文导致渲染慢。
  - `PROJECT_<i>_DETAIL`（原始链接）仅供后端生成二维码用，不直接展示给用户。
